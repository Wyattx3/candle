...
- WebSocket events use a discriminated `type` field. Full contract: `prompt`, `ping`/`pong`, `approval_request`/`approval_decision`/`approval_response`, `clarification_request`/`clarification_decision`/`clarification_response`, `task_started`, `status`, `thinking`, `thought_chunk`, `reasoning_chunk`, `tool_start`, `tool_end`, `security_notice`, `run_checkpoint`, `session_title`, `learning_update`, `genui`, `error`, `cancelled`. Keep this contract stable on both ends.
- UI streaming nodes are `text | reasoning | tool | approval | security | genui` (see `app/index.tsx` `AiStreamNode`).
- Inline generative UI: the client renders rich artifact cards in the chat stream from a `genui` node. The server emits `{ type: "genui", id, kind, data }` where `kind` is one of `insights | table | kanban | agent_action | plan | workers | locations` and `data` is the renderer payload (optional — renderers fall back to sample data). The client handler in `app/index.tsx` validates `kind` and pushes a `genui` node; `components/GenUI/GenUIBlock` dispatches to the matching renderer. The backend does not yet emit `genui` — wiring an agent tool that calls `emitEvent({ type: "genui", ... })` is the remaining step (define in `tools.ts`, register in `agent/llm.ts` `parentTools`, and thread `emitEvent` access through the run/session context).
